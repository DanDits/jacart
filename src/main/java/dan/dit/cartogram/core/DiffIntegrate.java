...
public class DiffIntegrate {
  ...
  public void diff_calcv(double t) {
    ...
    double[] rho = mapGrid.getRho().getOutputData();
    ...
    mapGrid.getGridvx().execute();
    mapGrid.getGridvy().execute();
  ...
  public void diff_integrate() {
    ...
    FftPlan2D gridvx_plan = mapGrid.getGridvx();
    FftPlan2D gridvy_plan = mapGrid.getGridvy();
        ...
        if (accept) {
          for (k = 0; k < lx * ly; k++) {
            ...
            if ((mid[k].x - eul[k].x) * (mid[k].x - eul[k].x) +
              (mid[k].y - eul[k].y) * (mid[k].y - eul[k].y) > mapGrid.getAbsoluteTolerance() ||
